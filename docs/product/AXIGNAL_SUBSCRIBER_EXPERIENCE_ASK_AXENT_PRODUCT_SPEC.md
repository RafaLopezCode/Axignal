...
## 2.1 Current canonical model

AXIGNAL canonizes **GPT-6 Luna (`gpt-6-luna`) as the current default cognitive model** for both of the following roles:

### A. AXIGNAL Brain / background cognitive work

**GPT-6 Luna via Batch** is the current default for asynchronous, high-volume cognitive work such as:
...
- economic-neighbourhood interpretation;
- targeted research batches;
- other bounded cognitive workloads generated by AXENT.

### B. Subscriber UX / Ask AXENT

**GPT-6 Luna via Standard Responses API** is the current default for interactive subscriber cognition such as:
...
- identify patterns in a bounded Xeed projection;
- contextual analysis;
- follow-up questions.

The model receives the **minimum relevant structured projection of the subscriber's authorized Xeed context**, not an indiscriminate dump of AXIGLAND.

## 2.2 Why GPT-6 Luna

At the date of this specification, OpenAI describes GPT-6 Luna as an efficient model for focused, high-volume tasks. Official model documentation lists Responses and Batch support, function calling, structured outputs, a 1,050,000-token context window and a 128,000-token maximum output. These are external operational facts, not product doctrine.

Current published Standard short-context pricing is:

- Input: **$0.10 / 1M tokens**
- Cached input: **$0.01 / 1M tokens**
- Output: **$0.50 / 1M tokens**

Batch and Flex are priced at 50% of applicable Standard rates. Cached-input pricing applies to cached input tokens; cache writes have a separate rate. For requests above 272K input tokens, input and cache rates are doubled and output pricing is 1.5x. Exact prices and thresholds are mutable and must be rechecked against official pricing documentation before implementation or budgeting.

These economics make the same model family unusually suitable for both AXIGNAL's asynchronous cognitive metabolism and low-cost contextual subscriber assistance.

## 2.3 Canonical does not mean hard-coded forever

GPT-6 Luna is the **current canonical default**, not a permanent domain dependency.
...
CognitiveProvider
    ...
    +-- OpenAI
          |
          +-- GPT-6 Luna [CURRENT DEFAULT]
```

Model identity must remain configuration/policy, not business-domain authority.

A future model may replace Luna only through an explicit evaluated model-policy change using representative AXIGNAL workloads.

> **MODEL_PROVIDER_IS_NOT_DOMAIN_AUTHORITY**

> **MODEL_OUTPUT_IS_NOT_CANONICAL_TRUTH**

> **LUNA_DOES_NOT_DIRECT_AXIGNAL**

> **AXIGNAL_USES_LUNA_AS_COGNITIVE_METABOLISM**
...
Every material or surprising graph element should expose:
...
GPT-6 Luna is invoked when natural-language cognition adds value.
...
GPT-6 Luna Standard with bounded context and appropriate reasoning effort.

## Level 3 — New investigation

Research request enters AXENT's governed research loop; asynchronous cognitive work may use GPT-6 Luna Batch.

This prevents the subscriber chatbot from becoming an uncontrolled research agent.

---

# 12. GPT-6 Luna execution policy

## 12.1 Standard / interactive
...
- model: `gpt-6-luna`
- processing: Standard
- reasoning effort: selected by policy according to interaction class
- streaming: preferred where UX benefits
- structured/tool calls: used where deterministic AXIGNAL retrieval is required

Interactive requests must optimize:
...
## 12.2 Batch / asynchronous

Default background cognitive surface:

- OpenAI Batch API
- model: `gpt-6-luna`

Use for work that does not require synchronous subscriber latency.

Batch is especially appropriate for Xeed germination and background AXENT research because the workload is asynchronous and high volume.
...
Cost, latency and quality must be measured by interaction class.

## 12.4 Long-context discipline

The 1.05M-token context window is capacity, not a target.
...
Initial policy may map all three to GPT-6 Luna.
...
Define contextual retrieval, GPT-6 Luna Standard policy, grounding, response structure, research escalation and telemetry.
...
GPT_6_LUNA_IS_CURRENT_DEFAULT_COGNITIVE_MODEL
GPT_6_LUNA_BATCH_IS_CURRENT_BACKGROUND_DEFAULT
GPT_6_LUNA_STANDARD_IS_CURRENT_SUBSCRIBER_DEFAULT
MODEL_SELECTION_IS_POLICY_NOT_DOMAIN_AUTHORITY
MODEL_CHANGE_REQUIRES_EVALUATION
...
# 26. Current GPT-6 Luna baseline

At document creation time, the official OpenAI API documentation identifies:

- Model ID: `gpt-6-luna`
- Positioning: efficient model for focused, high-volume tasks
- Context window: 1,050,000 tokens
- Max output: 128,000 tokens
- Standard short-context input: $0.10 / 1M tokens
- Standard cached input: $0.01 / 1M tokens
- Standard output: $0.50 / 1M tokens
- Batch/Flex: 50% of applicable Standard rates
- Responses API: supported
- Batch API: supported
- Function calling: supported
- Structured outputs: supported

Pricing and model capabilities are external operational facts and may change. They must be represented in runtime configuration/policy rather than hard-coded as permanent business doctrine.

Pricing is context-sensitive: the published short-context rates apply through 272K input tokens; above that threshold, input and cache rates are 2x and output rates are 1.5x. Cache writes are separately priced. Confirm current rates, applicable discounts and thresholds from official OpenAI pricing documentation when a runtime or budget is designed. These figures are a dated operational snapshot, not an entitlement or cost guarantee.

The canonical architectural decision is therefore not a permanent price assumption. It is:

> **GPT-6 Luna is AXIGNAL's current default cognitive model until an explicit evidence-based model-policy change supersedes it.**
...
> **a living external representation of their economic world that continuously becomes more complete, shows what changed, explains why it sees what it sees, and gives them AXENT whenever they want to investigate further.**
