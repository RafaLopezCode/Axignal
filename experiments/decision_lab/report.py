...
def render_report(result: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Cost estimate: {result.get('actual_cost_estimate', result.get('cost', 'UNKNOWN'))}",
            f"- Invoice cost: {result.get('invoice_cost', 'UNKNOWN')}",
            f"- Result: {result.get('result', result.get('outcome', 'INCONCLUSIVE'))}",
            f"- Result reasons: {result.get('result_reasons', 'NOT AVAILABLE')}",
